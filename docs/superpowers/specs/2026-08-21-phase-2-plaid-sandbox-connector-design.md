...
- **Navigation / dashboard:** the card profile is synthesised from the first enabled **credit** `bank_accounts` row, and is null when the owner has no enabled credit account — one Plaid Item usually carries a checking account too, and it is usually the older row, so the filter is on `type`, not on `created_at`. The hardcoded "Spark" label goes away; no card → the existing em-dash state.
...
- `PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ENV` (`sandbox | production`, default `sandbox`) join the zod schema, `.env.example` (Required in: prod once `PLAID_ENV=production`; not on preview), `ci/env.production.fixture` (dummy values, pinned by the fixture test) and the gitleaks value allowlist if the dummies are secret-shaped.
- `assertProductionSecurity`: when `PLAID_ENV === 'production'`, all three must be present (and `CRON_SECRET`, already required); **`PLAID_ENV === 'sandbox'` is refused in production** — a production deployment must never quietly talk to Sandbox.
- **Previews leave all three unset; Sandbox Link is exercised on a development machine.** Vercel builds and runs a preview with `NODE_ENV=production`, so the refusal above applies to previews too and a preview holding Sandbox keys does not boot. A preview with no Plaid at all is a supported deployment: the connections screen says Plaid is not configured, the Playwright journey covers connect → sync → inbox against a scripted bank in CI, and `next dev` with `PLAID_ENV=sandbox` is the supported way to drive real Link.
- **No `VERCEL_ENV === 'preview'` allowance**, deliberately. The assertion's value is that it makes one unconditional claim — a process with `NODE_ENV=production` will not talk to Sandbox. Keying it on a second variable demotes that to a claim about configuration: `VERCEL_ENV` is set by the platform, absent everywhere else, and settable by anyone who can set `PLAID_ENV`, so an env file copied from Vercel to a VPS would re-open the hole while the check reported itself satisfied. What the allowance buys — clicking Plaid's own Link UI on a shared URL — is not worth that.
- The provider factory reads env once; UI degrades to the "not configured" state rather than throwing.

*Amended after final review (2026-08-21): the original text said "Previews stay Sandbox by env scoping", which the boot assertion refuses. The review endorsed the code and the rule above replaces the sentence.*
